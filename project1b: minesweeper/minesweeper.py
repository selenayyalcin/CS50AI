...
class MinesweeperAI():
        ...
        for row in range(len(self.knowledge)):
            for column in range(row + 1, len(self.knowledge)):
                ...
                if sentence_row.cells.issubset(sentence_column.cells):
                    new = Sentence(sentence_column.cells - sentence_row.cells,
                                   sentence_column.count - sentence_row.count)
                    if new not in self.knowledge:
                        self.knowledge.append(new)

                elif sentence_column.cells.issubset(sentence_row.cells):
                    new = Sentence(sentence_row.cells - sentence_column.cells,
                                   sentence_row.count - sentence_column.count)
                    if new not in self.knowledge:
                        self.knowledge.append(new)
    ...
    def make_random_move(self):
        ...
        """
        moves = [(i, j) for i in range(self.height)
                 for j in range(self.width)
